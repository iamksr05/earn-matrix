...
contract TaskEscrow {
    ...
    // Track which tasks have been accepted (before funding)
    mapping(uint256 => bool) public taskAccepted;
    ...
    /**
     * @notice Worker accepts a task (on-chain acceptance)
     * @param taskId Task identifier from Supabase
     * @param posterAddress Address of the task poster (for verification)
     */
    function acceptTask(uint256 taskId, address posterAddress) external {
        require(posterAddress != address(0), "invalid poster");
        require(msg.sender != posterAddress, "poster cannot accept own task");
        require(!taskAccepted[taskId], "task already accepted");
        
        Escrow storage e = escrows[taskId];
        
        // If escrow already exists, check it matches
        if (e.poster != address(0)) {
            require(e.poster == posterAddress, "poster mismatch");
            require(e.worker == address(0) || e.worker == msg.sender, "worker already assigned");
        }
        
        // Set worker if not already set
        if (e.worker == address(0)) {
            e.worker = msg.sender;
        }
        
        // Set poster if not already set
        if (e.poster == address(0)) {
            e.poster = posterAddress;
        }
        
        taskAccepted[taskId] = true;
        
        emit TaskAccepted(taskId, msg.sender, posterAddress);
    }
