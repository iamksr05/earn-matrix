...
// ---------- writes ----------
/** Poster funds; can be done before worker is assigned using ZeroAddress. */
export async function fundEscrow(task) {
  ...
  const workerAddress = task.worker_wallet || ethers.ZeroAddress;
  const tx = await c.fund(task.id, workerAddress, { value });
