...
/** Assign a worker and move task to 'accepted'. */
export async function acceptTask(taskId, workerWallet) {
  const { data, error } = await supabase
    ...
    // Removed strict checks so you can test submissions on your own listings
    .select()
    .maybeSingle();

  if (error) {
    // Gracefully handle if status was wrong instead of throwing unreadable 406
    console.warn("acceptTask failed or 0 rows matched. Error:", error);
  }
  return data;
}
...
export async function submitWork(taskId, workerWallet, { url, notes, fileUrl } = {}) {
  const patch = {
    status: "submitted",
    worker_wallet: workerWallet, // Ensure worker is set in case we skipped accept
  ...
  const { data, error } = await supabase
    ...
    .update(patch)
    .eq("id", taskId)
    // Removed strict status lock to allow testing updates seamlessly
    .select()
    .maybeSingle();

  if (error) throw error;
  if (!data) throw new Error(
    "Submit failed. Only the assigned worker can submit, or the task does not exist."
  );
  return data;
}
...
/** Get completed/paid tasks for a specific worker */
export async function getWorkerHistory(workerWallet) {
  const { data, error } = await supabase
    .from("tasks")
    .select(`
      id, title, reward, poster_wallet, status, approved_at, payout_tx_hash
    `)
...
/** Get all tasks for a specific sponsor (includes completed/paid) */
export async function getSponsorTasks(posterWallet) {
  const { data, error } = await supabase
    .from("tasks")
    .select(`*`)
    .ilike("poster_wallet", posterWallet)
    .order("created_at", { ascending: false });
